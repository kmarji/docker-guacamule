...
RUN curl -SLO "https://github.com/just-containers/s6-overlay/releases/download/v2.2.0.3/s6-overlay-${ARCH}.tar.gz" \
  ...
  && tar -xf s6-overlay-${ARCH}.tar.gz -C /usr ./bin \
...
####RUN wget http://nz2.archive.ubuntu.com/ubuntu/pool/main/o/openssl/libssl1.1_1.1.1-1ubuntu2.1~18.04.20_amd64.deb && dpkg -i libssl1.1_1.1.1-1ubuntu2.1~18.04.20_amd64.deb
...
RUN curl -SLO "https://apache.org/dyn/closer.lua/guacamole/${GUAC_VER}/source/guacamole-server-${GUAC_VER}.tar.gz?action=download" \
  && mv guacamole-server-${GUAC_VER}.tar.gz?action=download guacamole-server-${GUAC_VER}.tar.gz \
  && tar -xzf guacamole-server-${GUAC_VER}.tar.gz \
  && cd guacamole-server-${GUAC_VER} \
  && ./configure --enable-allow-freerdp-snapshots --disable-dependency-tracking \
